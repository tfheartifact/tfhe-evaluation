...
use crate::shortint::server_key::{
    CompressedModulusSwitchConfiguration, LookupTableOwned,
    ModulusSwitchNoiseReductionKeyConformanceParams, ShortintBootstrappingKey,
};
...
impl CompressedServerKey {
    ...
    /// Prepare a standard [`ServerKey`] from the compressed form right before a batch of PBS
    /// operations.
    ///
    /// This keeps the original decompression API intact while exposing a dedicated entry point for
    /// experiments comparing the baseline server-key flow against a "compressed-then-decompressed"
    /// execution path.
    pub fn prepare_runtime_decompression_server_key(&self) -> ServerKey {
        self.decompress()
    }

    /// Experimental PBS path keeping the server key compressed until execution time.
    ///
    /// This preserves the existing [`Self::decompress`] + [`ServerKey`] workflow while offering a
    /// second code path to compare the impact of runtime decompression on PBS correctness.
    pub fn apply_lookup_table_with_runtime_decompression(
        &self,
        ct: &Ciphertext,
        acc: &LookupTableOwned,
    ) -> Ciphertext {
        let mut ct_res = ct.clone();
        self.apply_lookup_table_with_runtime_decompression_assign(&mut ct_res, acc);
        ct_res
    }

    /// In-place variant of [`Self::apply_lookup_table_with_runtime_decompression`].
    pub fn apply_lookup_table_with_runtime_decompression_assign(
        &self,
        ct: &mut Ciphertext,
        acc: &LookupTableOwned,
    ) {
        let decompressed_server_key = self.decompress();
        decompressed_server_key.apply_lookup_table_assign(ct, acc);
    }
...
mod tests {
    ...
    #[test]
    fn runtime_decompression_matches_standard_server_key_pbs() {
        let cks = ClientKey::new(PARAM_MESSAGE_2_CARRY_2_KS_PBS);
        ...
        for msg in 0..modulus {
            let ct = cks.encrypt(msg);

            let standard_res = sks.apply_lookup_table(&ct, &lut);
            let runtime_decompressed_res =
                compressed_sks.apply_lookup_table_with_runtime_decompression(&ct, &lut);

            assert_eq!(cks.decrypt(&standard_res), cks.decrypt(&runtime_decompressed_res));
        }
    }
}
